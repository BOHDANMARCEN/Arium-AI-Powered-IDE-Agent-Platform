...
import { ToolExecutionPool, ToolExecutor, ToolResultEvent } from "./toolExecutionPool";
...
export class ToolEngine {
  ...
  private executionPool: ToolExecutionPool;
  ...
  constructor(private eventBus: EventBus) {
    ...
    this.executionPool = new ToolExecutionPool({
      maxConcurrent: parseInt(process.env.TOOL_EXECUTION_POOL_SIZE || "5", 10),
      emitEvent: (event: ToolResultEvent) => {
        this.eventBus.emit("ToolResultEvent", event);
      },
    });

    // Cleanup rate limit store every 5 minutes
    this.cleanupInterval = setInterval(() => this.rateLimiter.cleanup(), 5 * 60 * 1000);
  ...
  /**
   * Execute tool asynchronously using the execution pool
   */
  executeAsync(
    toolId: string,
    args: Record<string, unknown>,
    ...
    // Create executor function for the execution pool
    const executor: ToolExecutor = async (input, signal) => {
      try {
        const result = await t.run(input as Record<string, unknown>, { eventBus: this.eventBus });
        if (signal.aborted) {
    ...
    return this.executionPool.execute(toolId, args, executor);
  }
