  ...
  private async executePython(
    ...
    return new Promise((resolve) => {
      ...
      child.on("close", () => {
        ...
        const memoryErrorMessage = this.extractMemoryError(stderr, stdout);

        if (!stdout.trim()) {
          resolve({
            ok: false,
            error: {
              type: memoryErrorMessage ? "MemoryError" : undefined,
              message: memoryErrorMessage || stderr.trim() || "Python runner returned no output",
            },
          });
          return;
        }
        ...
        try {
          ...
          if (memoryErrorMessage) {
            resolve({
              ok: false,
              error: {
                type: "MemoryError",
                message: memoryErrorMessage,
              },
            });
            return;
          }
...
`;
  ...
  private extractMemoryError(stderr: string, stdout?: string): string | null {
    const combined = `${stderr || ""}\n${stdout || ""}`;
    return combined.includes("MemoryError") ? combined.trim() : null;
  }
